...
const generateContrasts = require(path.resolve(__dirname, ('../generate-contrasts')));
const jsonToSassMap = require(path.resolve(__dirname, ('../json-to-sass-map')));
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--kod-${e(key)}`, value], [`--kod-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`kod.customProps.background.${colorScheme}`);
            const foreground = theme(`kod.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--kod-${e(key)}`, value], [`--kod-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--kod-${e(key)}`, value], [`--kod-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--kod-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--kod-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                kod  : {
                    customProps: {
